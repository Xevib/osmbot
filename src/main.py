...
def LanguageCommand(message,user_id,chat_id,user):
    ...
    bot.sendMessage(chat_id,_("Chose the language"), reply_markup={'keyboard': keyboard, 'one_time_keyboard': True})
    user.set_field(user_id, 'mode', 'setlanguage')
    return []

def SettingsCommand(message,user_id,chat_id,u):
    bot.sendMessage(chat_id,"Chose the settings",reply_markup={'keyboard': [['Language']],'one_time_keyboard': True})
    ...
    if updates['ok']:
        ...
        for query in updates['result']:
            try:
                ...
                print str(e)
                bot.sendMessage(chat_id, [gettext.gettext("Somthing failed please try it latter")], disable_web_page_preview=(not preview))
