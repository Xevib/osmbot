...
def pretty_tags(data):
    ...
    if 'wikidata' in tags:
        preview = True
        t += "\xF0\x9F\x93\x97 [{0}](https://www.wikidata.org/wiki/{1})".format(tags['wikidata'], urllib.quote(tags["wikidata"])) + "\n"
    if 'wikipedia' in tags:
        preview = True
        if ":" in tags["wikipedia"]:
            lang = str(tags['wikipedia'].split(":")[0])
            term = str(tags['wikipedia'].split(":")[1])
            t += "\xF0\x9F\x93\x92 [{0}](http://{1}.wikipedia.org/wiki/{2})".format(term,lang, urllib.quote(term)) + "\n"
        else:
            t += "\xF0\x9F\x93\x92 [{0}](http://wikipedia.org/wiki/{1})".format(tags['wikipedia'], urllib.quote(tags["wikipedia"])) + "\n"
